...
function Profile() {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      const res = await fetch(`${process.env.BASE_URL}/api/v1/users/avatar`, {
        method: "POST",
        body: formData,
        credentials: "include",
      });
  ...
  const HandleSubmitContact = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.BASE_URL}/api/v1/users/UserContact`, {
        method: "POST",
  ...
  const HandleSubmitInfo = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.BASE_URL}/api/v1/users/UserInfo`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
