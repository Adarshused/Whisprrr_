...
function Connectionspage(){
  ...
  // get all followers
    ...
    const FetchFollowers = async () => {
      try {
        const res = await fetch(`${process.env.BASE_URL}/api/v1/connection/getFollowers`, {
          method : "GET",
          credentials: "include"
        })
    ...
    const sorted_values=[...values].sort((a,b)=>b.upvote-a.upvote);
     useEffect(()=>{setuserDetail(sorted_values)},[])
      ...
      //  console.log(followee)
        try {
            const res = await fetch(`${process.env.BASE_URL}/api/v1/connection/follow`,{
              method: "POST",
       ...
       const HandleUnFollow = async(e) => {
        ...
        const followee = e.currentTarget.id
        try {
          const res = await fetch(`${process.env.BASE_URL}/api/v1/connection/unfollow`, {
            method: "POST",
            headers: {"Content-Type" : "application/json"},
            body: JSON.stringify({followee}),
            credentials: "include"
